...
use crate::quad::{DecQuad, DEC_QUAD_NEGATIVE_ZERO, DEC_QUAD_POSITIVE_ZERO};
...
impl TryFrom<DecQuad> for i8 {
  ...
  fn try_from(dq: DecQuad) -> Result<Self, Self::Error> {
    ...
    if (unsigned as i16) >= (i8::MIN as i16) && (unsigned as i16) <= (i8::MAX as i16) {
      Ok(unsigned as i8)
    } else {
      Err(())
    }
...
impl TryFrom<DecQuad> for u64 {
  ...
  ///
  fn try_from(dq: DecQuad) -> Result<Self, Self::Error> {
    let (encoded, mut unsigned) = unsafe { (dq.longs[0], (dq.bytes[8] * 10) as u128) };
